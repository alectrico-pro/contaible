...
(defrule distribucion-de-utilidades-del-ejercicio-anterior-a-cuenta-destino
   ...
   ( balance (ano ?ano_top))
   ( ticket  (numero ?numero))
   ( distribucion-de-utilidad (partida ?numero ) (cuenta-de-destino ?cuenta-de-destino))
   ( empresa (nombre ?empresa ))
   ( cuenta  (nombre utilidad ) (haber ?utilidad) (dia ?dia) (mes ?mes) (ano ?ano))
   ( cuenta  (nombre ?cuenta-de-destino) (haber ?haber))
   ( test (> ?utilidad 0))
   ( test (= ?haber 0))
=>
   ( assert (partida (numero ?numero) (mes ?mes) (dia ?dia) (ano ?ano) (descripcion (str-cat "Por traspaso de utilidad a Capital Social "  ?utilidad)) (actividad distribucion-utilidad)))
   ( assert (abono (electronico false) (tipo-de-documento traspaso) (cuenta ?cuenta-de-destino) (partida ?numero) (empresa ?empresa) (monto ?utilidad) (dia ?dia) (mes ?mes) (ano ?ano) (glosa (str-cat distribucion-utilidad- ?utilidad)) ))
   ( assert (cargo (electronico false) (tipo-de-documento traspaso) (cuenta utilidad) (partida ?numero) (empresa ?empresa) (monto ?utilidad) (dia ?dia) (mes ?mes) (ano ?ano) (glosa (str-cat distribucion-utilidad- ?utilidad))))
   ( assert (ccm (folio na) (partida ?numero) (tipo-documento traspaso) (monto-total ?utilidad)))
   ( printout t "Distribución Utilidad del Ejercicio Anterior "  ?utilidad crlf )
)
