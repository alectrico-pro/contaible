...
ARGV.each do |filename|
  begin
    ...
    if (doc.at_css('p.noindent'))
      doc.at_css('p.noindent').remove #eliminando los tails
    end
    # =doc.xpath("//a").remove
    #s =doc.at_css('p').remove
    ...
    when /htmlch/ # chapter
     if
      ...
      doc.at_css('h2.chapterhead').remove #eliminando los encabezados
     end
     if (d=doc.at_css('p'))
      #.content.gsub(/^\"/,'')
     end
    ...
    File.open(filename, 'w:utf-8') do |file| #utf-8 es importante para que
    #    se pueda leer desde ruby on rails
      file.write html
    end
  rescue => e
