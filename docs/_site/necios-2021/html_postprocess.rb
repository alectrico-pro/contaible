#!/usr/bin/env ruby
#http://stackoverflow.com/questions/4831711/incompatible-encodings-with-ruby-and-nokogiri-html
...
ARGV.each do |filename|
  begin
    doc = Nokogiri::HTML::Document.parse(IO.read(filename),nil, 'ISO-8859-1' )
    warn "#{doc.encoding}"
    ...
    # quitando tails
    if (t = doc.at_css('p.noindent'))
      t.remove
      warn "  p.noindent removed'"
    ...
    when /htmlch/ # chapter
      ...
      if (ff=doc.at_css('h2.chapterhead'))
        ff.remove
      end
    ...
    html.gsub!(/_______+/, '<hr>')
        ...
        #eliminando los frton de abajo#eliminando los frton de abajo

    html.gsub!(/<p>\[<a href=\"book_htmlch1.html\">front<\/a>\] \[up\]/,'')
    # <p>[<a href="book_htmlch1.html">front</a>] [up] </p>
    # eliminando front y up al final     
    ...
    #implantando image_tag
    html.gsub!(/<img class="h" src=\"(\S*)\" style="(...*)>/,
     '<div class="jaja" style= "\2">
         <%=image_tag(attachments["\1"].url, width: "150") %>  
      </div>')

  #implantando sidebarimage
    html.gsub!(/<img class="sidebarimage" width="150" src=\"(\S*)\">/,'<%=image_tag(attachments["\1"].url, width: "150") %>')
